...
def create_enemy():
    side = random.randint(0, 3)  # Lado desde el cual aparece la nave enemiga (0: arriba, 1: derecha, 2: abajo, 3: izquierda)
    if side == 0:  # Arriba
    ...
    enemies.append((x, y, angle, speed))
...
while running:
    ...
    # Crear nuevas naves enemigas aleatoriamente si hay menos de 10
    if len(enemies) < 10:
        create_enemy()
    
    # Actualizar la posición de las naves enemigas y verificar colisiones
    new_enemies = []
    for enemy in enemies:
        x, y, angle, speed = enemy
        x += math.cos(angle) * speed
        y += math.sin(angle) * speed
        if 0 <= x < screen_width and 0 <= y < screen_height:
            new_enemies.append((x, y, angle, speed))
            ...
            create_enemy()  # Crear una nueva nave enemiga si la actual sale del encuadre
    enemies = new_enemies
    ...
    for x, y, _, _ in enemies:
        pygame.draw.polygon(screen, white, [(x - 10, y + 10), (x + 10, y + 10), (x, y - 10)])
